...
local DeviceTypes = {
	SmallMobile = {width = 320, height = 568},   -- iPhone SE, small phones
	Mobile = {width = 375, height = 667},        -- Standard mobile
	LargeMobile = {width = 414, height = 896},   -- iPhone Pro Max, large phones  
	SmallTablet = {width = 768, height = 1024},  -- iPad Mini
	Tablet = {width = 834, height = 1194},       -- iPad Air
	LargeTablet = {width = 1024, height = 1366}, -- iPad Pro
	Desktop = {width = 1200, height = 800}       -- Desktop/laptop
}

local currentDeviceType = "Desktop" -- Default
local useMobileSizing = false
local useTabletSizing = false
...
local function detectDeviceType()
	...
	isPortrait = screenSize.Y > screenSize.X
	
	-- Determine device type based on screen dimensions
	if screenSize.X <= DeviceTypes.SmallMobile.width or screenSize.Y <= DeviceTypes.SmallMobile.height then
		currentDeviceType = "SmallMobile"
		useMobileSizing = true
		useTabletSizing = false
		screenScale = 0.8
	elseif screenSize.X <= DeviceTypes.Mobile.width or screenSize.Y <= DeviceTypes.Mobile.height then
		currentDeviceType = "Mobile"
		useMobileSizing = true
		useTabletSizing = false
		screenScale = 0.9
	elseif screenSize.X <= DeviceTypes.LargeMobile.width or screenSize.Y <= DeviceTypes.LargeMobile.height then
		currentDeviceType = "LargeMobile"
		useMobileSizing = true
		useTabletSizing = false
		screenScale = 1.0
	elseif screenSize.X <= DeviceTypes.SmallTablet.width or screenSize.Y <= DeviceTypes.SmallTablet.height then
		currentDeviceType = "SmallTablet"
		useMobileSizing = false
		useTabletSizing = true
		screenScale = 1.1
	elseif screenSize.X <= DeviceTypes.Tablet.width or screenSize.Y <= DeviceTypes.Tablet.height then
		currentDeviceType = "Tablet"
		useMobileSizing = false
		useTabletSizing = true
		screenScale = 1.2
	elseif screenSize.X <= DeviceTypes.LargeTablet.width or screenSize.Y <= DeviceTypes.LargeTablet.height then
		currentDeviceType = "LargeTablet"
		useMobileSizing = false
		useTabletSizing = true
		screenScale = 1.3
	else
	...
	if debugX then
		warn(string.format("Rayfield: Detected %s device (%dx%d) - Scale: %.1f - Portrait: %s", 
			currentDeviceType, screenSize.X, screenSize.Y, screenScale, tostring(isPortrait)))
	end
end

-- Dynamic sizing function based on device type
local function getResponsiveSize(baseWidth, baseHeight)
	local sizes = {
		SmallMobile = {width = math.floor(baseWidth * 0.85), height = math.floor(baseHeight * 0.8)},
		Mobile = {width = math.floor(baseWidth * 0.9), height = math.floor(baseHeight * 0.85)},
		LargeMobile = {width = math.floor(baseWidth * 0.95), height = math.floor(baseHeight * 0.9)},
		SmallTablet = {width = math.floor(baseWidth * 1.05), height = math.floor(baseHeight * 1.0)},
		Tablet = {width = math.floor(baseWidth * 1.1), height = math.floor(baseHeight * 1.05)},
		LargeTablet = {width = math.floor(baseWidth * 1.15), height = math.floor(baseHeight * 1.1)},
		Desktop = {width = baseWidth, height = baseHeight}
	}
	
	local deviceSize = sizes[currentDeviceType] or sizes.Desktop
	return UDim2.new(0, deviceSize.width, 0, deviceSize.height)
end

-- Dynamic text size calculation
local function getResponsiveTextSize(baseSize)
	return math.floor(baseSize * screenScale)
end

-- Dynamic drag offset calculation
local function getResponsiveDragOffset()
	local offsets = {
		SmallMobile = 120,
		Mobile = 140,
		LargeMobile = 150,
	...
	return offsets[currentDeviceType] or 255
end
...
local minSize = Vector2.new(320, 180)
...
FloatingButtonFrame.TextSize = getResponsiveTextSize(10)
...
local function Maximise()
	...
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = getResponsiveSize(380, 280)}):Play()
	TweenService:Create(Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = getResponsiveSize(380, 25)}):Play()
...
local function Unhide()
	...
	Main.Visible = true
	TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = getResponsiveSize(380, 280)}):Play()
	TweenService:Create(Main.Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = getResponsiveSize(380, 25)}):Play()
	...
	dragBar.Position = UDim2.new(0.5, 0, 0.5, getResponsiveDragOffset())
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	makeDraggable(Main, Topbar, false, {getResponsiveDragOffset(), getResponsiveDragOffset()})
	if dragBar then dragBar.Position = UDim2.new(0.5, 0, 0.5, getResponsiveDragOffset()) makeDraggable(Main, dragInteract, true, {getResponsiveDragOffset(), getResponsiveDragOffset()}) end
	...
	task.wait(0.1)
	TweenService:Create(Main, TweenInfo.new(0.6, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = getResponsiveSize(380, 280)}):Play()
	TweenService:Create(Main.Shadow.Image, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {ImageTransparency = 0.6}):Play()
...
-- Enhanced Safe Auto Fishing System (Hybrid Security)
RayfieldLibrary.CreateSafeAutoFish = function(config)
	config = config or {}
	local safeMode = config.safeMode or true
	local perfectChance = config.perfectChance or 70 -- 70% perfect, 30% human-like
	local minDelay = config.minDelay or 0.8
	local maxDelay = config.maxDelay or 2.5
	local useServerTime = config.useServerTime or true
	
	local autoFishEnabled = false
	local autoFishThread = nil
	
	-- Safe coordinates with human-like variation
	local function getSafeCoordinates()
		if safeMode and math.random(1, 100) > perfectChance then
			-- Human-like imperfect coordinates
			return {
				x = math.random(-1500, 1500) / 1000, -- -1.5 to 1.5
				y = math.random(500, 1000) / 1000,   -- 0.5 to 1.0
				timestamp = useServerTime and workspace:GetServerTimeNow() or (tick() + math.random(-0.5, 0.5))
			}
		else
			-- Perfect coordinates (but with slight variation)
			return {
				x = -1.238 + math.random(-50, 50) / 10000, -- Slight variation
				y = 0.969 + math.random(-20, 20) / 10000,  -- Slight variation
				timestamp = useServerTime and workspace:GetServerTimeNow() or 9999999999
			}
		end
	end
	
	-- Safe delay with human-like patterns
	local function getSafeDelay()
		local baseDelay = math.random(minDelay * 100, maxDelay * 100) / 100
		-- Add occasional longer pauses (like human would)
		if math.random(1, 20) == 1 then
			baseDelay = baseDelay + math.random(1, 3)
		end
		return baseDelay
	end
	
	local function toggleAutoFish(enabled)
		autoFishEnabled = enabled
		
		if enabled then
			autoFishThread = task.spawn(function()
				local consecutivePerfect = 0
				local maxConsecutive = math.random(3, 7) -- Vary max consecutive perfect casts
				
				while autoFishEnabled do
					pcall(function()
						-- Get safe fishing data
						local fishData = getSafeCoordinates()
						
						-- Force imperfect cast if too many consecutive perfect
						if consecutivePerfect >= maxConsecutive then
							fishData.x = math.random(-1500, 1500) / 1000
							fishData.y = math.random(500, 1000) / 1000
							consecutivePerfect = 0
							maxConsecutive = math.random(3, 7) -- Reset max
						end
						
						-- Track consecutive perfect casts
						if math.abs(fishData.x + 1.238) < 0.01 and math.abs(fishData.y - 0.969) < 0.01 then
							consecutivePerfect = consecutivePerfect + 1
						else
							consecutivePerfect = 0
						end
						
						-- Execute fishing with safe parameters
						local Rs = game:GetService("ReplicatedStorage")
						local net = Rs.Packages._Index["sleitnick_net@0.2.0"].net
						
						-- Equip rod if needed
						local char = game.Players.LocalPlayer.Character
						if char and not char:FindFirstChild("!!!EQUIPPED_TOOL!!!") then
							net["RF/CancelFishingInputs"]:InvokeServer()
							net["RE/EquipToolFromHotbar"]:FireServer(1)
							task.wait(0.1)
						end
						
						-- Safe fishing sequence
						net["RF/ChargeFishingRod"]:InvokeServer(fishData.timestamp)
						task.wait(0.05)
						net["RF/RequestFishingMinigameStarted"]:InvokeServer(fishData.x, fishData.y)
	...
	return {
		toggle = toggleAutoFish,
		isEnabled = function() return autoFishEnabled end,
		updateConfig = function(newConfig)
			for k, v in pairs(newConfig) do
				if k == "safeMode" then safeMode = v
				elseif k == "perfectChance" then perfectChance = v
				elseif k == "minDelay" then minDelay = v
				elseif k == "maxDelay" then maxDelay = v
				elseif k == "useServerTime" then useServerTime = v
				end
			end
		end
	}
